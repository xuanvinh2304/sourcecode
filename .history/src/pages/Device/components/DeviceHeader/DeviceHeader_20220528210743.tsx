...
const DeviceHeader: FC = () => {
  ...
  const onChange = (value: any) => {
    ...
    if(values !== '') {
      const result = infoThietBi.filter((item: any) => item.tenThietBi === values || item.tenDangNhap === values || item.loaiThietBi === values || item.maThietBi === values || item.diaChi === values);
      console.log(result);
      if(result[0] !== undefined) {
        setInfoThietBi(result);
      } else {
        const getThietBi = async () => {
          setInfoThietBi(thietBiInfo.docs.map((doc:any) =>({...doc.data(), id: doc.id})));
        }
        getThietBi();
      }
    } else {
